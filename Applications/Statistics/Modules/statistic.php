...
function statistic($module, $interface, $date, $start_time, $offset)
{
	...
	$module_str = '';
	$_modules=\Statistics\Lib\Cache::$modulesDataCache;
	ksort($_modules);
	foreach ($_modules as $mod => $interfaces) {
		if ($mod == 'WorkerMan') {
			continue;
		}
	...
	if (empty($interface)){
		...
		foreach (\Statistics\Lib\Cache::$modulesDataCache as $mod => $interfaces) {
			if ($mod== $module){
				foreach ($interfaces as $if) {
					$ifSheet[$if]=0;
					multiRequestStAndModules($module, $if, $date);
					if (is_array(\Statistics\Lib\Cache::$statisticDataCache['statistic'])) {
						foreach (\Statistics\Lib\Cache::$statisticDataCache['statistic'] as $ip => $st_str) {
							$last_date=false;
							$_tmp_arr=explode("\n",$st_str);
							foreach ($_tmp_arr as $row){
								...
								if ($c){
									...
									if (empty($success_series_data[$b])){
		...
		foreach ($success_series_data as $time => $count){
			$astr[]='['.($time*1000).','.$count.']';
		}
		foreach ($fail_series_data as $time => $count){
			$bstr[]='['.($time*1000).','.$count.']';
		}
		
		$success_series_data = implode(',', $astr);
		$fail_series_data = implode(',', $bstr);
		// last error log
		$log_data_sheet=array();
		foreach ($log_data as $if => $rows){
			$log_data_sheet[$if]='';
			foreach ($rows as $ip => $rows2){
				$log_data_sheet[$if].=$rows2['data'];
			}
		}
	}
